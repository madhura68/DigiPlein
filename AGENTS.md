...
- **Bouwvolgorde:** sprint S-2026-06-11-1 (M0) → -2 (M1) → -3 (M2; chat-integratie als slotstap). `get_context(product_id)` toont alle `active_sprints`; kies expliciet de sprint binnen de actuele opdracht en behoud de bouwvolgorde vanaf M0/ST-001 (story-codes ST-001 t/m ST-004 in sprint -1). Lees `get_sprint_context(sprint_id)` voor het compacte story-/taakoverzicht; voeg `task_id` alleen toe voor het volledige taakplan. Gebruik `get_ideas_context(product_id)` alleen voor ideeën. Geef uitsluitend expliciet bekende `agent.runtime` (CLAUDE/CODEX) en `agent.model_id` mee. Context is geen uitvoeropdracht.
